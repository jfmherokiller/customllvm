...
///
/// \file
/// \brief This file is part of the ZCPU Disassembler.
///
...
namespace {
    class ZCPUDisassembler final : public MCDisassembler {
    ...
    public:
        ZCPUDisassembler(const MCSubtargetInfo &STI, MCContext &Ctx,
                         std::unique_ptr<const MCInstrInfo> MCII)
                : MCDisassembler(STI, Ctx), MCII(std::move(MCII)) {}
    };
} // end anonymous namespace

static MCDisassembler *createZCPUDisassembler(const Target &T,
                                              const MCSubtargetInfo &STI,
                                              MCContext &Ctx) {
    std::unique_ptr<const MCInstrInfo> MCII(T.createMCInstrInfo());
    return new ZCPUDisassembler(STI, Ctx, std::move(MCII));
}

extern "C" void LLVMInitializeZCPUDisassembler() {
    // Register the disassembler for each target.
    TargetRegistry::RegisterMCDisassembler(TheZCPUTarget32,
                                           createZCPUDisassembler);
}

MCDisassembler::DecodeStatus ZCPUDisassembler::getInstruction(
    ...
    if (Opcode >= ZCPU::INSTRUCTION_LIST_END)
        return MCDisassembler::Fail;
    ...
    for (unsigned i = 0; i < NumFixedOperands; ++i) {
        ...
        switch (Info.OperandType) {
            ...
            case ZCPU::OPERAND_P2ALIGN:
            case ZCPU::OPERAND_BASIC_BLOCK: {
                if (Pos + sizeof(uint64_t) > Bytes.size())
                    return MCDisassembler::Fail;
    ...
    for (unsigned i = 0; i < NumExtraOperands; ++i) {
        if (Pos + sizeof(uint64_t) > Bytes.size())
            return MCDisassembler::Fail;
        if (Desc.TSFlags & ZCPUII::VariableOpIsImmediate) {
